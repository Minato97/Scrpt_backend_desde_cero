    ...
    if os.path.exists(seeders_src):
        ...
        for file in os.listdir(seeders_src):
            ...
            if file == "DatabaseSeeder.php":
                ...
                shutil.copy2(src_file, dst_file)
                print_success(f"✓ DatabaseSeeder.php reemplazado con orden de FK correcto")
            else:
                dst_file = os.path.join(seeders_dst, file)
                shutil.copy2(src_file, dst_file)
                seeder_count += 1

        print_success(f"✓ Seeders copiados ({seeder_count} seeders de tablas + DatabaseSeeder)")
    else:
        print_warning("No se encontró directorio de seeders generado")
...
def print_final_summary(config, backend_generated):
    ...
    if backend_generated:
        ...
        print(f"  ✅ Seeders con datos de prueba (10 registros por tabla)")
        print(f"  ✅ DatabaseSeeder.php con orden correcto de FK")
